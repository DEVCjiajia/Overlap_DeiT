...
class PatchEmbedding(nn.Layer):
    ...
    def __init__(self,
        ...
        # num patches
        self.num_patches = ((image_size - patch_size) // embed_stride + 1) * (
                    (image_size - patch_size) // embed_stride + 1)
...
class Deit(nn.Layer):
    def __init__(self,
        ...
        self.pos_embed = paddle.create_parameter(
            shape=[1, int(self.patch_embed.num_patches + 2), embed_dim],
            dtype='float32',
            default_initializer=nn.initializer.TruncatedNormal(std=.02))
...
def build_deit(config):
    ...
    model = Deit(image_size = config.DATA.IMAGE_SIZE,
                 ...
                 embed_stride = config.MODEL.stride,
                 patch_size = config.MODEL.TRANS.PATCH_SIZE,
                 embed_dim = config
                 .MODEL.TRANS.EMBED_DIM,
